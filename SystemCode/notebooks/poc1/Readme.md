...
Phase 9 begins with the offline `scripts/build_website_inventory.py` audit. The one-command `scripts/automate_web_rag_pilot.py` workflow verifies school pages and processes shared operator pages. It checkpoints after every page and atomically updates its files. Fetch failures have structured codes and retry policies: temporary failures wait until `next_retry_at` with exponential backoff, while permanent policy, safety, and content failures are skipped. Approved pages refresh by age; URLs and hashes classify changes, while retrieval age is labelled current, aging, stale, or unknown. Pass `--limit` and `--shared-limit` for resumable batches, `--refresh-after-days` to control refresh age, or `--fresh` for a rebuild. BM25 retrieval never mixes operator evidence unless explicitly requested, and webpage evidence remains disconnected from recommendation ranking. See `PHASE9_WEBPAGE_RAG.md` for details.
