...
class ApiStartupTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_nearest_chat_uses_postal_code_and_full_grounded_catalogue(
        ...
        distances = {"A": 1.2, "B": 0.3}
        mock_distances.side_effect = lambda schools, _postal_code: [
            {**school, "distance_km": distances[school["school_id"]]}
            for school in schools
        ]
        mock_get_many.return_value = [mock_all.return_value[1]]
        ...
        with patch.dict(os.environ, {
            ...
            async with ASGITestClient(main.app) as client:
                ...
                follow_up = await client.post("/api/preferences", json={
